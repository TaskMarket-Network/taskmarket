...
TaskMarket's minimal agent runtime (Phase 1, step 01-03). It provides the
...
policy-gated and idempotent execution path, and built-in observability.
...
- **Errors** (`src/errors.ts`): structured `AgentRuntimeError` /
  ...
  `TIMEOUT`, `IDEMPOTENCY_CONFLICT`, `EXECUTION_FAILED`, `INTERNAL`).

## Usage

```ts
import { createAgentRuntime, loadAgentRuntimeConfig } from '@taskmarket/agent-runtime';
...
schemas, success/failure paths, policy blocking, idempotency, health, and
metrics/log observability.
...
(input/output schemas, versioning, auth, health) is introduced in step 01-04.
