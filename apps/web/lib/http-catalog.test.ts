...
describe('catalogHttpStatusForErrorCode', () => {
  it('maps client error codes to 4xx statuses', () => {
    expect(catalogHttpStatusForErrorCode(MARKETPLACE_CATALOG_ERROR_CODES.REQUEST_INVALID)).toBe(400);
    ...
    expect(catalogHttpStatusForErrorCode(MARKETPLACE_CATALOG_ERROR_CODES.VERSION_CONFLICT)).toBe(409);
    expect(catalogHttpStatusForErrorCode(MARKETPLACE_CATALOG_ERROR_CODES.STATUS_TRANSITION)).toBe(422);
